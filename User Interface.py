...
class Ui_MainWindow(object):
    ...
    def create_map(self,path):
        ...
        for i in range(len(self.coor) - 1):
            self.distance2.append(great_circle(self.coor[i], self.coor[i + 1]).km)
        ...
        for i in range(len(self.data)):
            ...
            html = """
                        ...
                        """ % ((data.iloc[i]['name']),(data.iloc[i]['province']),
                               (data.iloc[i]['country']),(data.iloc[i]['lat']),(data.iloc[i]['lon']))


            folium.Marker([data.iloc[i]['lon'], data.iloc[i]['lat']],
                popup=html,
                icon=folium.Icon(color=self.color, icon='plane'),
    ...
    def tamam(self):
        ...
        threshold = 0
        list_for_sketch = main(self.data,index1,index2,threshold) #call warshall algorithm
        path = list_for_sketch[0][1]
        self.r = len(list_for_sketch)
